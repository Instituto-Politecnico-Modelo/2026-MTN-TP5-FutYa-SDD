...
- [ ] T014 Crear `2026-MTN-TP3-Back-Miceli-Tabuada/backEnd/src/main/java/com/example/backEnd/Entidad/Reserva.java` — entidad JPA: `id`, `fecha` (LocalDate), `estado` (`@Enumerated EstadoReserva`, default PENDIENTE), `motivoCancelacion` (VARCHAR 500, nullable), `canceladoPorAdmin` (boolean, default false), `canceladoPorUsuario` (`@ManyToOne(LAZY)` nullable a `Usuario`, solo para cancelación admin), `fechaCreacion` (Timestamp auto), `fechaModificacion` (Timestamp, nullable); `@ManyToOne(LAZY) usuario`, `@ManyToOne(LAZY) turno`, `@OneToOne(mappedBy="reserva") pago`; `@Table(uniqueConstraints = @UniqueConstraint(columnNames={"turno_id","fecha"}))` — garantiza SC-003
...
- [ ] T028 [US5][US6] Crear `2026-MTN-TP3-Back-Miceli-Tabuada/backEnd/src/main/java/com/example/backEnd/Service/ReservaService.java` — `crearReserva(turnoId, fecha, usuarioId)`: verifica disponibilidad, crea `Reserva` (PENDIENTE) + `Pago` (PENDIENTE) atómicamente en `@Transactional`, invoca internamente `PagoService.iniciarPago(reservaId)` y retorna `ReservaConPagoResponseDTO` con `pagoRedirectUrl`; `cancelarReserva(reservaId, usuarioId)`: valida propiedad + fecha no pasada; si anticipación > 24h llama `PagoService.reembolsar()` y marca REEMBOLSADO; si < 24h solo cancela sin reembolso; `cancelarReservaAdmin(reservaId, motivo, adminId)`: sin restricción de plazo, persiste `canceladoPorUsuario=admin`; `@Scheduled(fixedRate=300000) liberarReservasPendientes()`: busca PENDIENTE con `fechaCreacion < ahora - timeout` y las cancela
...
- [ ] T074 [US5] Crear `2026-MTN-TP3-Front-Miceli-Tabuada/src/pages/cliente/NuevaReserva.tsx` — stepper de 3 pasos: **Paso 1** turno/fecha (prellenado desde query params si vienen de Grilla); **Paso 2** info del clima (`ClimaResponse`; banner de aviso si `disponible=false`); **Paso 3** confirmar → `POST /api/reservas` y redirección directa usando `pagoRedirectUrl`; manejo de 409 (turno ya ocupado)
...
- [ ] T083 [US5] Crear `2026-MTN-TP3-Front-Miceli-Tabuada/src/services/pago.ts` — función `iniciarPago(reservaId): Promise<PagoIniciarResponse>` para reintentos de reservas pendientes (flujo alternativo); el flujo principal de `NuevaReserva` usa `pagoRedirectUrl` de `POST /api/reservas`
...
- [ ] T086 [US5] Conectar `NuevaReserva.tsx` con `reservas.ts` — Paso 2: llamar `getClimaParaTurno(fecha, hora)` con banner de fallback; Paso 3: llamar `createReserva()` y redirigir usando `pagoRedirectUrl` de la respuesta; manejo de 409 con mensaje "Turno no disponible"
